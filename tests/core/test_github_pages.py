...
def test_github_pages_delivery_writes_astro_content_post(tmp_path: Path) -> None:
    ...
    assert (
        'description: "Daily AI learning radar with 0 tech news items, '
        '0 GitHub repos, and 1 research paper."'
    ) in post
    assert "paper: 1" not in post
    assert "Product UI" in post
    assert "aurora-card" in post
    assert "# Aurora Unified Digest" not in post
    assert "Generated by Aurora from run" not in post
    assert "run-20260526T033547Z" not in post
